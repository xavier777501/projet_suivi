"""
Script pour finaliser la migration - convertir les champs ID restants (version simplifiée)
"""
...
def finalize_migration():
    print("=== Finalisation de la migration (version simplifiée) ===\n")
    
    try:
        with engine.connect() as connection:
            # 1. Vérifier si le champ identifiant existe déjà
            result = connection.execute(text("DESCRIBE utilisateur"))
            columns = result.fetchall()
            
            identifiant_exists = any(col[0] == 'identifiant' for col in columns)
            id_utilisateur_exists = any(col[0] == 'id_utilisateur' for col in columns)
            
            if id_utilisateur_exists:
            ...
            elif identifiant_exists:
                print("1. Le champ identifiant existe déjà, pas de renommage nécessaire")
            else:
                print("1. ERREUR: Ni id_utilisateur ni identifiant n'existent")
                return
            ...
            for table, column in tables_and_columns:
                ...
                # Vérifier si le champ existe déjà en varchar
                result = connection.execute(text(f"DESCRIBE {table}"))
                columns = result.fetchall()
                column_exists = any(col[0] == column for col in columns)
                column_type = next((col[1] for col in columns if col[0] == column), None)
                
                if column_type and 'varchar' in column_type.lower():
                    print(f"   ✓ Le champ {table}.{column} est déjà en varchar, saut de la conversion")
                    continue
                ...
                # Étape 2: Copier les données (conversion de int à varchar avec MySQL CONCAT)
                connection.execute(text(f"UPDATE {table} SET {column}_temp = CONCAT({column}, '')"))
