...
const CreateEspacePedagogique = ({ onClose, onSuccess }) => {
  ...
  const [filieres, setFilieres] = useState([]); // Pour l'affichage si besoin, ou filtrage
  ...
  const handleChange = (e) => {
    ...
    // Si on change la promotion, on peut déduire la filière (optionnel, pour UX)
  ...
  // Charger toutes les matières au début ou quand on change de filière
  // Pour simplifier, on charge tout si pas de filière sélectionnée
  useEffect(() => {
    // Si pas de filière active, on ne charge rien ou tout ? 
    // Mieux : quand on sélectionne une promo, on charge les matières.
  }, []);
